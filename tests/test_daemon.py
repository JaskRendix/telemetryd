...
async def test_load_config(tmp_path):
    ...
    d = TelemetryDaemon(p)
    assert d.config["devices"][0]["host"] == "h"
...
async def test_poll_device_initial(tmp_path):
    ...
    with patch.object(
        ...
        "fetch_metrics",
        return_value=[
            SNMPResponse(oid="1", name="m", value=100, snmp_type="COUNTER32")
        ],
    ):
...
@pytest.mark.asyncio
async def test_start_one_loop(tmp_path):
    cfg = {
    ...
    with patch.object(d, "poll_device", side_effect=fake_poll):
        ...
        with patch("asyncio.sleep", new=AsyncMock()):
            ...
            assert any(t.result() == "stop" for t in done)
